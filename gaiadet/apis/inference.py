...
def init_detector(config, checkpoint=None, device='cuda:0', cfg_options=None):
    ...
    # replace sync bn
    config.model.backbone.norm_cfg = dict(type='DynBN')
...
def inference_detector(model, img):
    ...
    try:
        ...
        if 'datasets' in cfg.data.test.keys():
            test_pipeline_cfg = cfg.data.test.datasets[0].pipeline
        else:
    ...
    if isinstance(img, np.ndarray):
        ...
        test_pipeline_cfg.pipeline[0].type = 'LoadImageFromWebcam'
    else:
